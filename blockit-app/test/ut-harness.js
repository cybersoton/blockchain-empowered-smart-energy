...
module.exports = {
  ...
  initialize: function(folder, callback) {
    ...
    // new this.BusinessNetworkConnection(cardStoreObj);
    const cardStore = require('composer-common').NetworkCardStoreManager
        .getCardStore( {type: 'composer-wallet-inmemory'} );
    this.adminConnection = new this.AdminConnection({cardStore: cardStore});
    this.businessNetworkConnection = new this
        .BusinessNetworkConnection({cardStore: cardStore});// cardType);

    // 4. Import the PeerAdmin Card to the Memory card store
    const peerAdminCardName = 'PeerAdmin';
    return this.adminConnection.importCard(peerAdminCardName, idCard)
        .then(() => {
          ...
          this.info =
      definition.metadata.packageJson.name +
      '@' +
      definition.metadata.packageJson.version;
          ...
          return this.adminConnection
              .start(this.businessNetworkDefinition
                  .getName(), this.businessNetworkDefinition
                  .getVersion(), startOptions);
        }).then((networkAdminCardMap) => {
          this.log('Start Successful - network admin card received!!');
          this.networkAdminCardName = `admin@${this.businessNetworkDefinition
              .getName()}`;

          // 9. Import the network admin card
          return this.adminConnection
              .importCard(this.networkAdminCardName, networkAdminCardMap
                  .get('admin'));
        }).then(() => {
          this.log('Imported the Network Admin Card!!! =' +
      this.networkAdminCardName);

          // 10. Connect the business network
          return this.businessNetworkConnection
              .connect(this.networkAdminCardName);
        }).then(() => {
          this.log('Business connection successful!!!');

          // 13. Invoke the callback function
          callback(this.adminConnection,
              this.businessNetworkConnection,
